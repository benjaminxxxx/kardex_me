<div class="space-y-6">
    <div>
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="{{ route('purchases.index') }}">Compras</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Registro de compra</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>
    <div class="grid gap-6 md:grid-cols-[380px_1fr]">
        ...
        <flux:card class="space-y-4">
            ...
            <div>
                ...
                <livewire:shared.entity-search-select entityType="supplier" fieldContext="purchase-supplier"
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <flux:input wire:model="documentSeries" label="Serie" placeholder="F001" />
                <flux:input wire:model="documentNumber" label="Número" placeholder="00123" />
            </div>
            ...
            <flux:textarea wire:model="notes" label="Glosa / Observación"
        ...
        <div class="space-y-4">
            <flux:card class="space-y-4">
                ...
                <livewire:shared.entity-search-select entityType="product" fieldContext="purchase-product"
                        ...
                        <flux:table.rows>
                            @forelse ($items as $index => $item)
                                <flux:table.row wire:key="item-{{ $index }}">
                                    ...
                                    <flux:table.cell>
                                        <flux:input type="number" step="1" wire:model.live="items.{{ $index }}.quantity"
                                            size="sm" class="w-20" />
                                    </flux:table.cell>

                                    <flux:table.cell>
                                        <flux:input type="number" step="0.01" wire:model.live="items.{{ $index }}.unit_cost"
                                            size="sm" class="w-24" />
                                    </flux:table.cell>

                                    <flux:table.cell>
                                        <flux:input type="number" step="0.01"
                                            wire:model.live="items.{{ $index }}.discount_percent" size="sm" class="w-16" />
                                    </flux:table.cell>

                                    <flux:table.cell>
                                        <flux:input type="number" step="0.01"
                                            wire:model.live="items.{{ $index }}.igv_percent" size="sm" class="w-16" />
                                    </flux:table.cell>
                            ...
                            @empty
                                <flux:table.row>
                                    <flux:table.cell colspan="100%" class="text-center">
                                        <flux:text class="text-muted italic">Agregue productos a la compra usando el
                                            buscador.
                                        </flux:text>
                                    </flux:table.cell>
                                </flux:table.row>
                ...
                <div class="rounded-lg bg-zinc-800 p-4 text-right space-y-1">
                    <div><span class="font-medium">Subtotal Neto:</span> S/. {{ number_format($this->subtotal, 2) }}
                    </div>
                    <div><span class="font-medium">IGV (Impuesto):</span> S/. {{ number_format($this->igvTotal, 2) }}
                    </div>
